  ...
  const debouncedSend = useCallback(
    debounce(async (input: string) => {
      if (input.trim()) {
        setIsLoading(true);
        try {
          const encodedApiKey = encodeURIComponent(API_KEY); // Ensure API key is encoded
          const response = await axios.post(
            `https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${encodedApiKey}`,
            {
              contents: [
                {
                  parts: [{ text: input }]
